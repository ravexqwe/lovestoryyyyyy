...
const userRoles = require('./src/models/UserRoles.js');
...
return sendLog({ content: `@here`, embeds: [embed] });
...
return } else {
KhonshuUrl(Config.ServerURL, Config.guildID, `${random}`)
}}, 1 * 500)})/** */
...
OtherGuard.on("presenceUpdate", async (eski, yeni) => {
 /*   const row = new MessageActionRow()
      .addComponents(   
         new MessageButton()
        .setCustomId("rolveristeaq")
      ...
      .setStyle("DANGER") ) /** */ 
const khonshupresence = yeni.clientStatus;
  const embed = new MessageEmbed();
  ...
  let kanal = OtherGuard.channels.cache.get(Config.Logs.kanalid)
  const roller = yeni.member.roles.cache.filter((e) => e.editable && e.name !== "@everyone" && bitfields.some((a) => e.permissions.has(a)));
  if (!yeni.user.bot && yeni.guild.id === Config.guildID && bitfields.some((e) => yeni.member.permissions.has(e)) ) {
    const sunucu = OtherGuard.guilds.cache.get(Config.guildID);
    if (sunucu.ownerID === yeni.user.id) return;
   //// if(["",""].some === yeni.user.id) return;
    

    if (khonshupresence.web) {
...
if(button.clicker.member.id !== sunucu.ownerID) return;
  ...
  .setThumbnail(yeni.user.displayAvatarURL({ dynamic: true, size: 2048 }))
  kanal.send({components: null,  embeds: [embedy]}); 
...
button.guild.members.ban(yeni.user.id, { reason: `Tarayıcıdan giriş yapmak | Yetkili: ${button.clicker.member.user.tag}` , days:1}).catch(() => {});
  ...
  if (khonshupresence.mobile) {
    ...
    await veri.roles.map(e => yeni.member.roles.add(e, "Platformunu değiştirdiği için yetkili rolleri geri verindi :)"));
      ...
      .setDescription(`${yeni.user.toString()} üyesi tarayıcıdan çıktığı için bütün yetkili rolleri geri verildi. :)`)
      kanal.send({components: null,  embeds: [embedx]}); 
   
    }
 if (khonshupresence.desktop) {
  const veri = await userRoles.findOne({ guildID: Config.guildID, userID: yeni.user.id });
  if (!veri) return;
  await veri.roles.map(e => yeni.member.roles.add(e, "Platformunu değiştirdiği için yetkili rolleri geri verindi :)"));
     await userRoles.findOneAndDelete({ guildID: Config.guildID, userID: yeni.user.id });
     let embedxx = new MessageEmbed()
     .setDescription(`${yeni.user.toString()} üyesi tarayıcıdan çıktığı için bütün yetkili rolleri geri verildi. :)`)
     kanal.send({components: null,  embeds: [embedxx]}); 
   } else {
    const veri = await userRoles.findOne({ guildID: Config.guildID, userID: yeni.user.id });
    if (!veri) return;
    let embedxxx = new MessageEmbed()
    .setDescription(`${yeni.user.toString()} üyesi tarayıcıda çevrimdışı moduna geçtiği için yetkilerini geri vermedim. :)`)
    kanal.send({components: null,  embeds: [embedxxx]}); 
 }
...
/*async function KhonshuUrl(vanity, token) {
let random = Config.Guards[Math.floor(Math.random() * Config.Guards.length)];
